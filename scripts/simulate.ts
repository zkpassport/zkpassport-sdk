...
async function main() {
  if (process.argv[2] === 'mobile') {
    ...
    const { onDomainVerified, notifyAccept, notifyReject, notifyDone } = await zkPassportProver.scan(scannedUrl, {
      keyPairOverride: {
        ...
        publicKey: new Uint8Array([
          3, 202, 45, 95, 176, 97, 188, 130, 46, 26, 69, 197, 152, 237, 220, 8, 6, 156, 55, 254, 254, 9, 96, 71, 169,
          10, 127, 249, 203, 125, 180, 136, 170,
        ]),
      },
    })
    ...
    const queryBuilder = await zkPassport.request({
      ...
      keyPairOverride: {
        privateKey: new Uint8Array([
          ...
          214, 94, 222, 43, 145, 55, 168, 230, 253,
        ]),
        publicKey: new Uint8Array([
